...
import type { NormalizedSchedule } from '../../src/core/daily_schedule/types';

const schedule: NormalizedSchedule = {
  kind: 'mixed',
  count: 4,
  items: [
    { id: 'active', sourceId: 'active', date: '2026-09-17', start: '10:00', end: '11:00', isTimed: true },
    { id: 'next-a', sourceId: 'next-a', date: '2026-09-17', start: '11:30', end: '12:00', isTimed: true },
    { id: 'next-b', sourceId: 'next-b', date: '2026-09-17', start: '11:30', end: '12:30', isTimed: true },
    { id: 'untimed', sourceId: 'untimed', date: '2026-09-17', isTimed: false },
  ],
};
...
describe('projectHomeSchedule', () => {
  it('derives Now and all simultaneous Up Next items from the canonical schedule', () => {
    ...
    expect(result.now.map(item => item.id)).toEqual(['active']);
    expect(result.upNext.map(item => item.id)).toEqual(['next-a', 'next-b']);
  ...
  it('does not treat point or untimed items as active without a canonical end time', () => {
    const pointOnly: NormalizedSchedule = {
      ...
      count: 2,
      items: [
        { id: 'point', sourceId: 'point', date: '2026-09-17', start: '10:30', isTimed: true },
        { id: 'untimed', sourceId: 'untimed', date: '2026-09-17', isTimed: false },
      ],
    };
