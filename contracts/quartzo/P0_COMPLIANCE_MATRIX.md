...
| --- | --- | --- | --- |
| P0.1 bootstrap | `AGENT_BOOTSTRAP.md`, `agents.md`, `.agents/AGENTS.md` | `test/agent_contract_bootstrap_test.dart`, `tool/agent_preflight.dart` | architecture and preflight gates |
| P0.2 scheduler documentation | parent contract plus `contracts/quartzo/scheduler/vectors.json` | exact repeat-type and expected-output assertions in `test/obsidian_companion_contracts_test.dart` | contract fixture gate |
| P0.3 shared settings | `QuartzoSharedSettings`, `SharedSettingsRepository`, `SettingsNotifier.reloadSharedSettingsFromVault`, `contracts/quartzo/shared_settings/v1.md` | `test/shared_settings_repository_contract_test.dart` migration, vault-retarget, canonical fixture, Object Identification persistence, malformed, external-change and no-write-loop tests | shared settings gate |
...
| P0.8 DONE vault interoperability | `contracts/quartzo/object_fixtures/fixtures.json`, `contracts/quartzo/object_fixtures/coverage.json`, `contracts/quartzo/object_fixtures/priority_vectors.json`, `contracts/quartzo/resource_capture/vectors.json`, canonical model serializers and Resource capture policies | 32 model-level parse -> serialize -> parse fixtures, 1 documented raw/read-only `daily_note` fixture, 8 parse/mutate/roundtrip mutation fixtures in `test/obsidian_companion_contracts_test.dart`, plus executable Resource source-detection and duplicate-policy vectors in `test/resource_capture_contract_test.dart` | interop and fixture gates |
