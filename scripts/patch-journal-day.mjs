...
function replaceExact(from, to, label) {
  ...
  if (source.indexOf(from, first + from.length) >= 0) throw new Error(`Expected one ${label}`);
...
`  private renderJournal(container: HTMLElement): void {
    ...
    if (projection.moodEntryCount > 0) {
      ...
      summary.textContent = `${projection.moodEntryCount} mood ${projection.moodEntryCount === 1 ? 'entry' : 'entries'} stored in the Daily Note.`;
