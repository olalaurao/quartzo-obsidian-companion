...
14. **Permissão real para limpeza de duplicados:** `safe cleanup available` só pode ser oferecido quando todo candidato que o plano pretende mover para a lixeira tiver `capabilities.canTrash == true` para a conta autenticada. Em candidatos byte-idênticos que todos batem com o local, uma única cópia não-trashable deve ser preservada e as redundantes trashable podem ser removidas. Se o plano exigir remover qualquer cópia sem permissão, a ambiguidade continua bloqueada/manual e a UI deve mostrar a permissão de cada candidato.
